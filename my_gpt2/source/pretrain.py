...
def load_model_for_run(args, device, master_process):
    ...
    init_meta = {
        ...
        "source_val_loss": ckpt.get("val_loss", None),
...
def main():
    ...
    model, init_meta = load_model_for_run(args, device, master_process)
    model.to(device)
    ...
    if master_process:
        with open(os.path.join(log_dir, "run_config.txt"), "w") as f:
